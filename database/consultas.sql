...
FROM
    ...
    JOIN Participacao ON (
        ...
        AND Participacao.nome_pessoa IN ('Robert Downey Jr.', 'Ícaro Silva')
    ...
    JOIN Producao ON (
        ...
        AND Producao.nome_organizacao IN ('Marvel Studios', 'Storytel')
    )
...
FROM
    ...
    LEFT JOIN Indicacao ON (
        Participacao.id_participacao = Indicacao.id_participacao
    );
...
FROM
    ...
    LEFT JOIN (
        Mod_Comentario
        JOIN Comentario ON (
            Mod_Comentario.id_comentario = Comentario.id_comentario
        )
